...
import { TrendingUp, TrendingDown, Volume2, DollarSign, Star, Plus, Zap } from "lucide-react";
import { OptimizedStockApiService, StockPrice } from "@/lib/optimizedStockApi";
...
export function OptimizedStockDiscovery() {
  ...
  useEffect(() => {
    const fetchStocks = async () => {
      ...
      try {
        ...
        // 使用優化的批量 API
        const stockData = await OptimizedStockApiService.getMultiplePrices(symbols);
        
        // 根據不同標籤排序
        let sortedStocks = [...stockData];
        switch (activeTab) {
          case 'gainers':
            sortedStocks.sort((a, b) => (b.changePercent || 0) - (a.changePercent || 0));
            break;
          case 'losers':
            sortedStocks.sort((a, b) => (a.changePercent || 0) - (b.changePercent || 0));
            break;
          case 'active':
  ...
  // 預加載熱門股票
  useEffect(() => {
    OptimizedStockApiService.preloadPopularStocks();
  }, []);
